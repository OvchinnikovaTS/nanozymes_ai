...
## Дизайн ML системы - nanozymes_ai (MVP). вер.1.1 </h1>
...
- Почему станет лучше, чем сейчас, от использования ML `Product Owner` & `Data Scientist` В каждой статье от 10 до 50 страниц, на изучение одной статьи тратится до 4 часов. Разрабатываемый сервис должен позволять уменьшать это время до 2 часов за счет information retrieval текста.
...
- Краткое описание БТ и ссылки на детальные документы с бизнес-требованиями `Product Owner`
  ...
  Создаем сервис, позволяющий в документах выделять данные выбранного пользователем типа. Например, сервис должен найти в тексте статьи инструкцию по синтезу химического вещества, в тексте документации к станку с ЧПУ - инструкцию для диагностики неполадок и т.п.

  Проектируемый user case:

![User story]((https://github.com/Rustemhak/nanozymes_ai)/{path}/image.png)

<i> 
  пользователь открывает сервис <br>
  в поле для выбора файла указывает путь к файлу статьи <br>
   <ul> 
     <li> указывает путь к файлу на локальном компьютере </li>
     <li> перетаскивает файл статьи с локального компьютера в поле выбора файла (Drag-and-drop)</li> 
   </ul>
  ...
  суммаризированный текст отображается на экране <br>
</i>
<br>
...
- Описание результата с точки зрения качества кода и воспроизводимости решения `Data Scientist`
  ...
  Данные, загружаемые пользователем, будут <b>временно*</b> храниться в векторной базе данных.
  Модель будет <b>дообучаться*</b> итеративно.

  Для предикта загружаемые пользователем данные
  > - передаются <b>модели распознавания*</b>  в формате JSON по API
...
- Что делаем с технической точки зрения: рекомендательная система, поиск аномалий, прогноз, оптимизация, и др. `Data Scientist`   База данных по нанозимам. Система распознавания релевантного задаче содержания научных статей. Классификация научных статей
